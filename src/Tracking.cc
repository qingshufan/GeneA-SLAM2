...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, shared_ptr<PointCloudMapping> pPointCloud, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mpPointCloudMapping( pPointCloud ), mpKeyFrameDB(pKFDB),
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    //Added offset via depth variance
    OffsetCurrentMask();
...
static std::string pointToString(const cv::Point2f& point) {
    return std::to_string(point.x) + "," + std::to_string(point.y);
}
void Tracking::RunGeneA(int& nmatches, std::vector<cv::KeyPoint>& all_red_keypoints){
    GeneA genea;
    std::vector<cv::KeyPoint> all_selected_keypoints;
    genea.runGeneA(mCurrentFrame.mvKeysUn, all_selected_keypoints,all_red_keypoints);

    std::unordered_map<std::string, bool> smap;
    // std::cout << "ask: " << all_selected_keypoints.size() << std::endl;
    // std::cout << "mku: " << mCurrentFrame.mvKeysUn.size() << std::endl;
    for (const auto& kp : all_selected_keypoints) {
        smap[pointToString(kp.pt)] = true;
    }

    std::vector<cv::KeyPoint> temps;
    std::vector<MapPoint*> temps2;
    temps.reserve(mCurrentFrame.mvKeysUn.size());
    temps2.reserve(mCurrentFrame.mvKeysUn.size());

    for (size_t i = 0; i < mCurrentFrame.mvKeysUn.size(); ++i) {
        auto it = smap.find(pointToString(mCurrentFrame.mvKeysUn[i].pt));
        if (it != smap.end() && it->second) {
            temps.push_back(mCurrentFrame.mvKeysUn[i]);
            temps2.push_back(mCurrentFrame.mvpMapPoints[i]);
        }
    }
    mCurrentFrame.mvKeysUn = std::move(temps);
    mCurrentFrame.mvpMapPoints = std::move(temps2);
    nmatches = mCurrentFrame.mvKeysUn.size();
    mCurrentFrame.N=nmatches;
}
bool Tracking::TrackWithOpticalFlow()
{
    ...
    int nmatches = matcher.SearchByOpticalFlow(mCurrentFrame, mLastFrameLK, mImGray, mImGrayLastLK, merged_mask);
    ...
    //---------- added GeneA ---------- 
    std::vector<cv::KeyPoint> all_red_keypoints;
    // GeneA genea;
    // std::vector<cv::KeyPoint> all_selected_keypoints;
    // std::vector<std::vector<cv::KeyPoint>> clusters = genea.ae_dbscan(mCurrentFrame.mvKeysUn, all_selected_keypoints);
    // for (const auto &cluster : clusters) {
    //     std::vector<cv::KeyPoint> selected_keypoints;
    //     std::vector<cv::KeyPoint> red_keypoints;
    //     std::vector<bool> best_x;

    //     genea.testGA(cluster, selected_keypoints, red_keypoints, best_x);
    ...
    // nmatches = mCurrentFrame.mvKeysUn.size();
    // mCurrentFrame.N=nmatches;
    RunGeneA(nmatches,all_red_keypoints);
    ...
    //draw cluster
    // for(const auto& keypoint:all_selected_keypoints){
    //     cv::circle(imgWithLKPoints,keypoint.pt, 5, cv::Scalar(0, 255, 0), 1);
    //     cv::circle(imgWithLKPoints,keypoint.pt, 1, cv::Scalar(0, 255, 0), -1);
    // }
    ...
    //         cv::circle(imgWithLKPoints, keypoint.pt, 5, color, 1);
    //         cv::circle(imgWithLKPoints, keypoint.pt, 1, color, -1);
    //     }
    // }



    //draw removed keypoints
    for(const auto& keypoint:all_red_keypoints){
        cv::circle(imgWithLKPoints,keypoint.pt, 8, cv::Scalar(0, 0, 255), 1);
        cv::circle(imgWithLKPoints,keypoint.pt, 1, cv::Scalar(0, 0, 255), -1);
    }
    ...
    int baseline = 0;
    stringstream s;
    s << "Matches: "<<filteredKeysUn.size()<<", Removed : " << all_red_keypoints.size();
    cv::Size textSize = cv::getTextSize(s.str(), cv::FONT_HERSHEY_PLAIN, 1, 1, &baseline);
    cv::putText(imgWithLKPoints, s.str(), cv::Point(5, imgWithLKPoints.rows - 5), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(255, 255, 255), 1, 8);
...
pcl::PointCloud< PointCloudMapping::PointT >::Ptr  generatePointCloudTest(KeyFrame* kf, cv::Mat& color, cv::Mat& depth)
 {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    mpPointCloudMapping->insertKeyFrame(pKF, this->mImRGB, new_mImDepth);
...
void Tracking::OffsetCurrentMask(){
    DepthOffset depthOffset;
    mImOffsetMask = depthOffset.offsetMask(mImRGB, mImDepth2);
    mImMapMask = depthOffset.mapMask(mImRGB, mImDepth2);
}

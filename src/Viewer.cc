...
Viewer::Viewer(System* pSystem, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Tracking *pTracking, shared_ptr<PointCloudMapping> pPointCloud, const string &strSettingPath, Settings* settings):
    both(false), mpSystem(pSystem), mpFrameDrawer(pFrameDrawer),mpMapDrawer(pMapDrawer), mpTracker(pTracking),mpPointCloudMapping( pPointCloud ),
...
void Viewer::drawPointCloud() {
    pcl::PointCloud<pcl::PointXYZRGBA>& clouds = *(mpPointCloudMapping->globalMap);
    if (!clouds.empty()) {
        ...
        for (size_t i = 0; i < clouds.size(); ++i) {
            try {
                ...
                const auto& point = clouds[i];
                if (std::isnan(point.x) || std::isinf(point.x) ||
                    std::isnan(point.y) || std::isinf(point.y) ||
                    std::isnan(point.z) || std::isinf(point.z)) {
                    std::cerr << "Point at index " << i << " has invalid coordinates: (" 
                              << point.x << ", " << point.y << ", " << point.z << ")" << std::endl;
                    continue;
                }
                if (point.r < 0 || point.r > 255 ||
                    point.g < 0 || point.g > 255 ||
                    point.b < 0 || point.b > 255) {
                    std::cerr << "Point at index " << i << " has invalid color: (" 
                ...
                std::cerr << "Exception occurred at point index " << i << ": " << e.what() << std::endl;
            }
        }
        glEnd();
    }
...
std::mutex frameDrawerMutex;
cv::Mat Viewer::GetFrameImage(float trackedImageScale) {
...
void Viewer::Run()
{
    ...
    //**** qingshufan modified code start ****
    pangolin::Var<bool> menuDenseMap("menu.Dense Map",true,true);
    pangolin::Var<bool> menuSaveFig("menu.Save Fig",false,false);
    ...
    pangolin::Var<bool> menuKeep("menu.Keep",true,true);
    //**** qingshufan modified code end ****
    ...
    while(1)
    {
        ...
        std::lock_guard<std::mutex> lock(frameDrawerMutex);
        ...
        if (menuDenseMap){
            std::lock_guard<std::mutex> lock(mpPointCloudMapping->cloudsMutex);
            if(!mpPointCloudMapping->denseMap) mpPointCloudMapping->denseMap=true;
            drawPointCloud();
        }else{
            if(mpPointCloudMapping->denseMap) mpPointCloudMapping->denseMap =false;
        }
        pangolin::FinishFrame();

        if(menuSaveFig){
            pangolin::SaveWindowOnRender("window");
            d_cam.SaveOnRender("camera");
            std::cout << "save window and camera to image" << std::endl;
            menuSaveFig=false;
        }
        //**** qingshufan modified code end ****

        cv::Mat toShow = GetFrameImage(trackedImageScale);
        ...
        if (!imLK.empty()) {
            cv::imshow("GeneA-SLAM2(NGD-SLAM): Current Frame", mpFrameDrawer->DrawLKFrame(1.0));
            cv::waitKey(mT);
        }

        cv::Mat imMask = mpFrameDrawer->DrawMaskFrame(1.0);
        if (!imMask.empty()) {
            cv::imshow("Predicted Mask", mpFrameDrawer->DrawMaskFrame(1.0));
            cv::waitKey(mT);
        }

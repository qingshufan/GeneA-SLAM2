...
std::mutex mMutexRightFrames;
cv::Mat FrameDrawer::DrawRightFrame(float imageScale)
{
    unique_lock<mutex> lock(mMutexRightFrames);
    cv::Mat im;
    vector<cv::KeyPoint> vIniKeys; // Initialization: KeyPoints in reference frame
    vector<int> vMatches; // Initialization: correspondeces with reference keypoints
    vector<cv::KeyPoint> vCurrentKeys; // KeyPoints in current frame
...
{
    unique_lock<mutex> lock(mMutexLK);
    return mImLK;
}
...
std::mutex mMutexMaskFrame;
cv::Mat FrameDrawer::DrawMaskFrame(float imageScale)
{
    unique_lock<mutex> lock(mMutexMaskFrame);
    return mImMask;
}
